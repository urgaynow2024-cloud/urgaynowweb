  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <AdminStatCard title="Staff" value={staffCount} icon="👥" description="Team members" />
        <AdminStatCard title="Group Photos" value={groupCount} icon="📷" description="Community moments" />
        <AdminStatCard title="Announcements" value={announcementCount} icon="📣" description="Published posts" />
        <AdminStatCard title="Events" value={eventCount} icon="📅" description="Scheduled events" />
      </div>

      {/* Content Overview */}
      <div className="grid gap-6 lg:grid-cols-2">
        <AdminCard title="Recent Activity" description="Latest changes across your content">
          {activity.length === 0 ? (
            <AdminEmptyState icon="📭" title="No recent activity" description="Start creating content to see activity here" />
          ) : (
      ...
      <AdminCard title="Quick Actions" description="Create new content">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Link href="/admin/staff/new" className="btn-secondary">+ Add Staff</Link>
          <Link href="/admin/announcements/new" className="btn-secondary">+ Announcement</Link>
          <Link href="/admin/events/new" className="btn-secondary">+ Event</Link>
          <Link href="/admin/group-photos/new" className="btn-secondary">+ Group Photo</Link>
        </div>
      </AdminCard>
    </div>
  );
