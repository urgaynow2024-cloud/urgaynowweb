    ...
    <div>
          ...
          {(["all", "review"] as const).map((f) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-sm font-medium capitalize transition-colors ${
                filter === f ? "bg-white text-ink-900 shadow-sm dark:bg-ink-900 dark:text-white" : "text-ink-500 hover:text-ink-800 dark:hover:text-ink-200"
              }`}
            >
              {f === "all" ? "All photos" : "Needs review"}
            </button>
          ))}
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          title={items.length === 0 ? "No group photos yet" : "No photos match your filters"}
          description={items.length === 0 ? "Upload your first group photo to get started." : "Try adjusting your search or filters."}
